...
int Task_lteTurboDecode_a2(
    // __v6148i8 softin2_x_B,__v6148i8 softin2_y_B ,
    __v6148i8 softin2_x_B,__v6148i8 softin2_y_B,__v6148i8 e_p_B,
    __v4096i8 state0_8, __v6148i16 alphain_N,
  ...
  */
  ...
  d1_B = vsadd(softin2_x_B,softin2_y_B,MASKREAD_OFF,B);   //22
  d1_B = vsadd(d1_B,a_p_B,MASKREAD_OFF,B);    //23
  d1_B = vsra(d1_B,1,MASKREAD_OFF,B);   //24
  d1_B = vmul(d1_B,3,MASKREAD_OFF,B);   //25
  d1_B = vsra(d1_B,1,MASKREAD_OFF,B);   //26
  //d3_B=d1_B; d1_B使用时用减号

  d2_B = vrsub(softin2_x_B,softin2_y_B,MASKREAD_OFF,B);   //27
  d2_B = vsadd(d2_B,a_p_B,MASKREAD_OFF,B);  //28
  d2_B = vsra(d2_B,1,MASKREAD_OFF,B);   //29
  d2_B = vmul(d2_B,3,MASKREAD_OFF,B);   //30
  d2_B = vsra(d2_B,1,MASKREAD_OFF,B);   //31
  //d4_B=d2_B; d2_B使用时用减号


  softin2_x_B = vsadd(softin2_x_B,0,MASKREAD_OFF,B);    //32
  softin2_y_B = vsadd(softin2_y_B,0,MASKREAD_OFF,B);    //33
  ...
  d1_B = vsadd(d1_B, 0,MASKREAD_OFF,B);       //34
  d2_B = vsadd(d2_B, 0,MASKREAD_OFF,B);       //35
  ...
  // for(short k=1; k<2;k++){
    ...
    */
    ...
    for(int i=0; i<SW; i++){
      ...
      // printf("d1 = %d\t",&d1);
      // printf("d2 = %d\n",&d2);
      vbrdcst(d1_B_ksub1,(-1)*d1,MASKREAD_OFF,8);
      vbrdcst(d2_B_ksub1,(-1)*d2,MASKREAD_OFF,8);
      vbrdcst(d3_B_ksub1,d1,MASKREAD_OFF,8);
      vbrdcst(d4_B_ksub1,d2,MASKREAD_OFF,8);
      
      vshuffle(atemp1_8temp,SW_tempindex,atemp1_8,SHUFFLE_GATHER,8);
      vshuffle(atemp2_8temp,SW_tempindex,atemp2_8,SHUFFLE_GATHER,8);

      atemp1_8temp = vsadd(atemp1_8temp,0,MASKREAD_OFF,8); 
      atemp2_8temp = vsadd(atemp2_8temp,0,MASKREAD_OFF,8); 

      vsgt(a_add_d1_index,ones,MASKREAD_OFF,MASKWRITE_ON,8);
      atemp1_8temp = vsadd(atemp1_8temp,d1_B_ksub1,MASKREAD_ON,8); 
      atemp2_8temp = vsadd(atemp2_8temp,d3_B_ksub1,MASKREAD_ON,8); 

      // atemp1_8temp = vsadd(atemp1_8temp,0,MASKREAD_OFF,8); 
      // atemp2_8temp = vsadd(atemp2_8temp,0,MASKREAD_OFF,8); 

      vsgt(a_add_d2_index,ones,MASKREAD_OFF,MASKWRITE_ON,8);
      atemp1_8temp = vsadd(atemp1_8temp,d2_B_ksub1,MASKREAD_ON,8); 
      atemp2_8temp = vsadd(atemp2_8temp,d4_B_ksub1,MASKREAD_ON,8); 

      vsgt(a_add_d3_index,ones,MASKREAD_OFF,MASKWRITE_ON,8);
      atemp1_8temp = vsadd(atemp1_8temp,d3_B_ksub1,MASKREAD_ON,8); 
      atemp2_8temp = vsadd(atemp2_8temp,d1_B_ksub1,MASKREAD_ON,8); 

      vsgt(a_add_d4_index,ones,MASKREAD_OFF,MASKWRITE_ON,8);
      atemp1_8temp = vsadd(atemp1_8temp,d4_B_ksub1,MASKREAD_ON,8); 
      atemp2_8temp = vsadd(atemp2_8temp,d2_B_ksub1,MASKREAD_ON,8); 

      vshuffle(atemp1_8,SW_tempindex,atemp1_8temp,SHUFFLE_SCATTER,8);
      vshuffle(atemp2_8,SW_tempindex,atemp2_8temp,SHUFFLE_SCATTER,8);

      
      SW_tempindex = vsadd(SW_tempindex,8,MASKREAD_OFF,8); 
    }
    atemp1_8 = vsadd(atemp1_8,0,MASKREAD_OFF,length);     //98
    atemp2_8 = vsadd(atemp2_8,0,MASKREAD_OFF,length);     //99
      


    //step3:max(atemp1_8,atemp2_8),作为a[k]
    vsgt(atemp1_8,atemp2_8,MASKREAD_OFF,MASKWRITE_ON,length);    //atemp1_8小于atemp2_8 100
    ...
    atemp1_8 = vxor(atemp1_8, atemp1_8, MASKREAD_ON, length);     //101
    atemp1_8 = vsadd(atemp1_8,0, MASKREAD_OFF, length);   //102
    // atemp1_8 = vsadd(atemp1_8, atemp2_8, MASKREAD_ON, length);   //得到a[k]
    atemp1_8 = vsadd(atemp1_8,atemp2_8, MASKREAD_ON, length);   //得到a[k] 103
    atemp = vsadd(atemp1_8,0, MASKREAD_OFF, length);  //104
    ...
    for(short i=0;i<SW;i++){
      ...
      // printf("tmp1=%d\t",&tmp1);
      // printf("tmp2=%d\n",&tmp2);
      vbrdcst(a_B_tempindex,k + i*kminus,MASKREAD_OFF,1);
  ...
  // softin1_x_B_out = vsadd(softin2_x_B,0,MASKREAD_OFF,B);
  ...
  vreturn(a1_B,B, a2_B,B, a3_B,B, a4_B,B, a5_B,B, a6_B,B, a7_B,B, a8_B,B, softin2_x_B,B, d1_B,B,d2_B,B , a_p_B,B, e_p_B,B );
  // vreturn(a1_B,B, a2_B,B, a3_B,B, a4_B,B, a5_B,B, a6_B,B, a7_B,B, a8_B,B, b1_B,B, b2_B,B, b3_B,B, b4_B,B, b5_B,B, b6_B,B, b7_B,B, b8_B,B, softin2_x_B,B, d1_B,B,d2_B,B , a_p_B,B, e_p_B,B );
