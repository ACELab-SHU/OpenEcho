...
// } __attribute__((aligned(64))) int_struct;
 ...
 int Task_ltePRBS(
     ...
     __v2048i16 seq2_trans_table_0_vec, __v2048i16 seq2_trans_table_1_vec, __v2048i16 seq2_trans_table_2_vec,
     __v2048i16 seq2_trans_table_3_vec, __v2048i16 seq2_trans_table_4_vec, __v2048i16 seq2_trans_table_5_vec,
     __v2048i16 seq2_trans_table_6_vec, short_struct nCellID, 
     int_struct nRNTI,short_struct qNum,short_struct slotNum,short_struct input_sequence_length
    // short_struct slotNum,short_struct qNum
  )
 {
  ...
  // int RNTI = nRNTI.data;
  // short q = qNum.data;
  // short ns = slotNum.data;
  // short sequenceLength = input_sequence_length.data;
   printf("NID = %d\n",&NID);
   printf("RNTI = %d\n",&RNTI);
   printf("q = %d\n",&q);
   printf("ns = %d\n",&ns);
  ...
  // printf("q = %hd\n",&q);
  // printf("ns = %hd\n",&ns);
  // printf("sequenceLength = %hd\n",&sequenceLength);
   int init2 = (RNTI*(1<<14) + q*(1<<13) + ns/2*(1<<9) + NID )% (1 << 31);
  printf("init2 = %d\n",&init2);
  ...
  //  printf("sequenceLength = %hd\n",&sequenceLength);
   seq = vsadd(seq, 0, MASKREAD_OFF, sequenceLength);   

  //  char word[15] = "PRBS finished";
  //  printf("----------- %s -----------\n",&word);       
